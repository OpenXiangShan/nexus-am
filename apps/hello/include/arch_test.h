...
.macro RVTEST_CODE_BEGIN inst_init inst_code_begin
  #.align UNROLLSZ
  .section .text.init;
  .globl \inst_init;                                                  \
  \inst_init:
  ...
  rvtest_prolog_done:
     ...
     LA (x5, \inst_code_begin);
     LA (x6, \inst_code_begin);
  ...
  .globl \inst_code_begin
  \inst_code_begin:
.endm

// --------------------------------- CODE BEGIN w/ TRAP HANDLER END -----------------------------//

.macro RVTEST_CODE_END inst_code_end
  .align 4; 
  .global \inst_code_end
  \inst_code_end:
#ifdef rvtest_mtrap_routine
  .option push
  .option norvc 
