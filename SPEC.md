...
* `_Device`代表一个设备描述结构体，包含设备id、设备名称和控制寄存器的读写函数。
...
* `extern _Device _devices;`描述系统中的设备。最后一个设备保证device id为0。

### 支持的设备

设备id为32位十六进制数，前16位为平台ID，后16位为设备ID。
...
| 设备ID      | 设备描述     | 设备功能                                     |
| --------- | -------- | ---------------------------------------- |
| 0000 ac01 | 性能计数器    | 若干32位寄存器，编号从0开始                          |
| 0000 ac02 | AM调试输出   | 向0号寄存器写入字节输出                             |
| 0000 ac03 | AM定时器    | 从0号寄存器读出系统启动到当前的毫秒数(32bit)               |
| 0000 ac04 | AM键盘控制器  | 从0号寄存器读取，全0表示无事件，第16位=1表示是keydown事件      |
| 0000 ac05 | AM显示控制器  | 向1 2 3 4 5写入参数，向0写入非0数值绘图。向0写入0重绘屏幕      |
| 0000 0080 | PCI控制器   | 寄存器为configuration space地址(bus, slot, func, offset组成) |
| 8086 00dd | ATA磁盘控制器 | x86，映射到7f0开始的端口                          |
