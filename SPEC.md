...
* `_RegSet`代表体系结构相关的寄存器组。`SYSCALL_REG`
...
| 设备ID      | 设备描述     | 设备功能                                     |
| --------- | -------- | ---------------------------------------- |
| 0000 0001 | 性能计数器    | 若干32位寄存器，编号从0开始                          |
| 0000 0002 | AM调试输出   | 向0号寄存器写入字节输出                             |
| 0000 0003 | AM定时器    | 从0号寄存器读出系统启动到当前的毫秒数(32bit)               |
| 0000 0004 | AM键盘控制器  | 从0号寄存器读取，全0表示无事件，第16位=1表示是keydown事件      |
| 0000 0005 | AM显示控制器  | 向1 2 3 4 5写入参数，向0写入非0数值绘图。向0写入0重绘屏幕      |
